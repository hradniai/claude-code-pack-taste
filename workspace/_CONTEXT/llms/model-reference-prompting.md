...
**GPT-6 Astra** (`gpt-6-astra`, GA 2026-09-03) is the current flagship. GPT-5.6 (Sol/Terra/Luna) remains live below it; the table below still shows the GPT-5.5 family's technical attributes as the reference baseline. Source: [OpenAI model guidance](https://developers.openai.com/api/docs/guides/latest-model)
...
Lineup clarifications: **"GPT-5.5 Instant" is a ChatGPT product name, NOT a separate API model** (the API flagship is `gpt-5.5`, ChatGPT alias `chat-latest`). `gpt-5.5-pro` / `gpt-5.4-pro` are real deep-reasoning API models (Responses API only). A cheaper **GPT-5.4 family** sits below 5.5 (`gpt-5.4`, `gpt-5.4-mini`, `gpt-5.4-nano`); `gpt-5.4-nano` is the going-forward replacement for `gpt-4.1-nano`. **Update: the GPT-5.6 generation (Sol/Terra/Luna) shipped 2026-07-09** and is now previous-generation; reasoning remains a mode of the model, not a separate brand. **Update 2026-08-06:** `chat-latest` was updated to a newer snapshot and may no longer point to `gpt-5.5`; pin a specific model in production. Source: [OpenAI model guidance](https://developers.openai.com/api/docs/guides/latest-model); [OpenAI changelog](https://developers.openai.com/api/docs/changelog)

Status / deprecation (developers.openai.com/api/docs/deprecations): GPT-4o, GPT-4.1, GPT-4.1 mini, o3, o4-mini **retired from ChatGPT 2026-02-13**; API stays live with sunsets - **`gpt-4.1-nano` deprecates 2026-10-23** (-> `gpt-5.4-nano`), **`o3-mini` / `o4-mini` 2026-10-23**, **`o3` 2026-12-11** (-> `gpt-5.5`). Base `gpt-4.1` / `gpt-4.1-mini` have no confirmed API sunset yet.

### Anthropic / Claude (current: Sonnet 5 + Opus 5, July 2026)

From the authoritative `claude-api` reference + live platform.claude.com docs (2026-07-31). **Claude Opus 5** (`claude-opus-5`) is now the current/default Opus (GA 2026-07-24). **Claude Sonnet 5 is the current/default Sonnet** (GA); **Claude Fable 5.1 is the newest Fable model** (above Opus tier) but is **disabled by policy - see `model-lineup.md`**. Source: [Claude Platform release notes](https://platform.claude.com/docs/en/release-notes/overview)

| | Claude Fable 5 | Claude Opus 5 | Claude Opus 4.8 | Claude Opus 4.7 | Claude Sonnet 5 | Claude Haiku 4.5 |
|---|---|---|---|---|---|---|
| **API ID** | `claude-fable-5` | `claude-opus-5` | `claude-opus-4-8` | `claude-opus-4-7` | `claude-sonnet-5` | `claude-haiku-4-5` |
| **Context** | 1M | 1M | 1M | 1M | 1M | 200K |
| **Max output** | 128K | 128K sync; 300K Batches | 128K | 128K | 128K | 64K |
| **Reasoning** | Adaptive ALWAYS on (omit the param; `{type:"disabled"}` 400s) | ON by default; `{type:"disabled"}` 400 with xhigh/max (allowed only <= `high`); effort low->max | Adaptive only (effort low->`xhigh`->`max`), default OFF | Adaptive only (effort low->`xhigh`->`max`), default OFF | Adaptive, ON by default (effort low->`xhigh`->`max`, default `high`) | None (no extended thinking, no effort) |
| **Sampling** | `temperature`/`top_p`/`top_k` -> 400, omit | -> 400, omit | -> 400, omit | -> 400, omit | non-default -> 400, omit | Standard |
| **Structured output** | Native (`output_config.format`) | Native | Native | Native | Native | Native |
| **Tool calling** | Native | Native | Native | Native | Native | Native |
| **Prompt format** | XML tags | XML tags | XML tags | XML tags | XML tags | XML tags |
| **Best for** | Hardest reasoning + longest-horizon agentic (disabled by policy) | **Current default Opus:** hardest long-horizon agentic, self-verifies work, delegates to subagents readily | Previous-gen Opus, still fully supported | Previous-gen Opus, fully supported | Current default Sonnet: near-Opus coding/agentic at Sonnet cost; best value | Real-time, high-volume, cost-sensitive |

**Claude Opus 5** (`claude-opus-5`) is now the current/default Opus (GA 2026-07-24). Opus 4.8 is previous-gen, still fully supported. Opus 4.6 (`claude-opus-4-6`) and Sonnet 4.6 (`claude-sonnet-4-6`) are also previous-gen, still supported. Aliases: bare `opus` -> current Opus (5), bare `sonnet` -> current Sonnet (5); on Bedrock/Vertex/Foundry specify the full ID. `claude-opus-4-1` **retired 2026-08-05** (API now returns error; migrate to `claude-opus-5`); `claude-opus-4-0` / `claude-sonnet-4-0` fully retired. Verify dates via the `claude-api` migration guide / `platform.claude.com` before quoting to a client.
...
> Status: retired from ChatGPT 2026-02-13; API live. `gpt-4.1-nano` deprecates 2026-10-23 (-> `gpt-5.4-nano`). New work targets GPT-5.x unless cost-constrained.
...
### Anthropic Claude (Fable 5 / Opus 5 / Opus 4.8 / 4.7 / Sonnet 5 / Sonnet 4.6)
...
| Agentic coding (hardest) | GPT-6 Astra; Claude Opus 5 (`xhigh`); GPT-5.6 Sol / 5.5 Pro | Frontier long-horizon execution |
